...
const Game = () => {
	...
	const handleLeaveRoom = async () => {
		if (!gameContext?.currentRoom) return;
		try {
			await gameContext.leaveRoom(gameContext.currentRoom.id);
	...
	return (
		<div className="flex flex-col gap-4 p-4">
			{/* Show typing race when race has started */}
					...
					{gameContext?.currentRoom && (
						<div className="bg-green-100 p-4 rounded">
							...
							{gameContext.countdownSeconds !== null && (
								<div className="mt-4 text-4xl font-bold text-center text-orange-600">
									{gameContext.countdownSeconds}
								</div>
							)}
